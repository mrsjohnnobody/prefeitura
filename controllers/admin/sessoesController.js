...
class SessaoController {
    ...
    async viewSessoesId(req, res){
        ...
        const sessaoValidate = await Sessao.findOne({where: {id: req.params.id, CamaraId: camara.id}});
        if(!sessaoValidate)
          return res.redirect("/admin/sessoes");
        
        const sessao = await Sessao.findByPk(
          id,
          { include: [
        ...
        res.render("admin/sessoes/sessaoIdPage", { sessao, parlamentares, prefeito, camara });
    }
    ...
    async addSessoes(req, res){
        try {
            ...
            return res.status(201).json({ status: "success", message: "Sessão cadastrada com sucesso", sessao: sessaoReturn })
        } catch (error) {
            console.log(error)
            return res.status(500).json({ status: "false", message: "Não foi possível realizar essa operação, tente novamente mais tarde" })
        }
    ...
    async deleteSessao(req, res) {
        try{
          ...
          if(!req.params.id)
            return res.status(200).json({ status: "false", message: "Não foi possível remover esta sessão" });
          
          const sessao = await Sessao.findOne({where: {id: req.params.id, CamaraId: camara.id}});
          ...
          return res.status(200).json({ status: "success", message: "Sessão removida com sucesso"});
    ...
    async editSessao(req, res) {
      try{
        ...
        if(!id)
          res.status(200).json({ status: "false", message: "Sessão não encontrada"});
        ...
        return res.status(200).json({ status: "success", message: "Sessão editada com sucesso"});
