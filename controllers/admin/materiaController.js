...
class MateriaController {
    async viewEditMateriaById(req, res){
        try {
          ...
          if(!req.params.id)
            return res.redirect("/admin/sessoes")
          ...
          res.render("admin/sessoes/materiaIdPage", { materia, sessao, autores, camara });
        } catch (error) {
            console.log(error)
    ...
    async deleteTramite(req, res) {
      try{
        ...
        if(!req.params.id)
          return res.status(200).json({ status: "false", message: "Não foi possível remover este trâmite" });
        ...
        return res.status(200).json({ status: "success", message: "Trâmite removido com sucesso"});
    ...
    async createTramite(req, res) {
      try{
        ...
        return res.status(200).json({ status: "success", message: "Trâmite cadastrado com sucesso", tramite: tramiteReturn, sessao })
    ...
    async deleteMateria(req, res) {
      try{
        ...
        if(!req.params.id)
          return res.status(200).json({ status: "false", message: "Não foi possível remover esta matéria" });
        
        const materia = await Materia.findOne({where: {id: req.params.id, CamaraId: camara.id}});
        ...
        return res.status(200).json({ status: "success", message: "Matéria removido com sucesso"});
    ...
    async createMateria(req, res) {
      try{
        ...
        return res.status(200).json({ status: "success", message: "Matéria cadastrada com sucesso", materia: materiaReturn })
