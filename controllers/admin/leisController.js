    ...
    try {
      ...
      if (!req.params.id)
        return res
          ...
          .json({
            status: "false",
            message: "Não foi possível remover esta lei",
          });

      const lei = await Leis.findOne({
      ...
      return res
        .status(200)
        .json({ status: "success", message: "Lei removida com sucesso" });
    } catch (error) {
      //salvar error em table
      return res
        .status(500)
  ...
  async createLei(req, res) {
    try {
      ...
      return res
        ...
        .json({
          status: "success",
          message: "Lei cadastrada com sucesso",
          lei: leiReturn,
        });
    } catch (error) {
      //salvar error em table
